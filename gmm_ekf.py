...
class Gmm_EKF(object):
    def __init__(self, dt, T, \
        ...
        self.current_output = self.x.copy()
    ...
    def run(self, measured_ranges=None, control_inputs=None):
        ...
        for k in range(self.N):
            ...
            if(self.debug):
                ...
                print('x2_est = [%.3f; %.3f]' %(self.x[0,0], self.x[1,0]))
...
class Robot(object):
    ...
    # returns p(z^|x^)
    def get_likelihood(self, z, x):
        # get a new measurement given the updated step
        self.lidar.wall_loc_x = x[0]
        self.lidar.wall_loc_y = self.wall.get_y(x[0])
        likelihood = self.lidar.get_likelihood(x)
        try:
            ...
            import pdb; pdb.set_trace()
            p = likelihood(6.0) # this means we're over
        return p
...
class Lidar(object):
    ...
    def __init__(self, sensor_inst_ang=90., wall_loc=3.0, meas_max=6.0, \
                 a1=.9, a2=0.0, a3=.0, a4=.1, norm_sig=0.1):
        self.sensor_inst_ang = sensor_inst_ang
        self.wall_loc_x = 0. #[m]
        self.wall_loc_y = 0. #[m]
        self.meas_max = meas_max # max measurement of the range sensor [m]
        self.a1, self.a2, self.a3, self.a4, self.norm_sig = a1,a2,a3,a4,norm_sig
    ...
    def get_range(self, x):
        ...
        # get the new noise distribution (because it varies with the nominal meas.)
        self.pdf, self.bins  = self.noise_dist(ideal_meas, \
                            a1=self.a1, a2=self.a2, a3=self.a3, a4=self.a4, norm_sig=self.norm_sig)
    ...
    def noise_dist(self, x_true, a1=1., a2=1., a3=1., a4=.1, norm_sig=1., exp_lambda=1., uni_delta=0.5, plot=False):
        N = 100
